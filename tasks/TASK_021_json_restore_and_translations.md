# TASK_021 — Naprawa JSON + description_en + values_en

## Kontekst

Pliki `devices/*.json` zostały uszkodzone przez TASK_019 (urwane stringi).
Ten task:
1. Przywraca czyste pliki z git
2. Dodaje `description_en` i `values_en` za pomocą skryptu Python (bezpieczna metoda)
3. Przebudowuje `devices-data.js`
4. Dodaje obsługę `values_en` w `calculator.js`

---

## Krok 1 — Przywróć devices/*.json z git

```bash
git checkout 620cff5 -- devices/
```
...
## Krok 2 — Uruchom skrypt tłumaczeń

Zapisz poniższy skrypt jako `tasks/add_translations.py` i uruchom go z katalogu głównego projektu.
...
VALUES_EN = {
    ...
    # TLeadSensorSelect drv_cool specific
    ('TLeadSensorSelect', '3'): {
        'TSL_T4 — czujnik T4 (złącze DRV)': 'TSL_T4 — T4 sensor (DRV connector)',
        'TSL_T4 — czujnik T4 (temperatura pomieszczenia)': 'TSL_T4 — T4 sensor (room temperature)',
        'TSL_T4 — czujnik T4 (powietrze przed wymiennikiem)': 'TSL_T4 — T4 sensor (air before heat exchanger)',
...
# drv_cool specific TLeadSensorSelect
VALUES_EN[('TLeadSensorSelect', '0')] = {'TSL_TNS — tylko odczyt': 'TSL_TNS — read-only'}
VALUES_EN[('TLeadSensorSelect', '3')] = {
    'TSL_T4 — czujnik T4 (złącze DRV)': 'TSL_T4 — T4 sensor (DRV connector)',
    'TSL_T4 — czujnik T4 (temperatura pomieszczenia)': 'TSL_T4 — T4 sensor (room temperature)',
    'TSL_T4 — czujnik T4 (powietrze przed wymiennikiem)': 'TSL_T4 — T4 sensor (air before heat exchanger)',
    'TSL_T4 — czujnik T4 (powietrze przed wymiennikiem wodnym)': 'TSL_T4 — T4 sensor (air before water heat exchanger)',
...
def add_translations(obj, device_key):
    ...
    if isinstance(obj, dict):
        # Opis urządzenia
        if 'name' not in obj and 'description' in obj and 'description_en' not in obj:
            pass  # opis urządzenia — pomijamy (nie mamy klucza name)
...
Uruchom:
```bash
python3 tasks/add_translations.py
```
...
## Krok 4 — Obsługa values_en w calculator.js

W funkcji `renderValueInfo(reg)` znajdź fragment wyświetlający wartości rejestru.
Typowy wygląd:

```js
// PRZED:
    if (reg.values) {
      // ... buduje listę wartości z reg.values ...
    }

// PO:
    const regValues = (currentLang === 'en' && reg.values_en) ? reg.values_en : reg.values;
    if (regValues) {
      // ... identyczny kod jak poprzednio, ale z regValues zamiast reg.values ...
    }
```

Dokładny kod zależy od aktualnej implementacji. Przeczytaj `renderValueInfo` i zastąp
odwołania do `reg.values` przez `regValues` (tylko wewnątrz bloku wartości, nie ruszaj reszty).
...
1. JSON OK ✓
2. Otwórz `frontend/index.html` w EN, oblicz dla T-box/M-box
3. Kliknij rejestr z wartościami (np. WorkMode w dowolnym urządzeniu) —
   opisy wartości po angielsku: "WM_OFF — device off" ✓
4. Przełącz na PL — opisy wartości po polsku ✓
...
```
feat(i18n): add description_en and values_en to all device registers via Python script
```
